...
If not, playoffs will take as much height as necessary to display all matches.  
But it may have a negative side effect: the actual height will start changing on navigation  
(because by default playoffs tries to reduce the height according to which rounds are visible. Thus it plays better with not-so-tall wrappers).  
